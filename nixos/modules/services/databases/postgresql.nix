...
let
  ...
  in pkgs.writeShellApplication {
    ...
    text = ''
      ...
      if [[ "$(tr --delete '[:space:]' <"${cfg.databaseDir}/current/PG_VERSION")" == "${cfg.finalPackage.psqlSchema}" ]]; then
      ...
      pushd "${cfg.dataDir}"
      ${cfg.finalPackage}/bin/pg_upgrade ${lib.cli.toGNUCommandLineShell { } args} ${lib.escapeShellArgs cfg.upgrade.extraArgs}
      touch .post_upgrade
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.postgresql = {
      ...
      preStart = ''
        ...
        ln -sfn "${cfg.finalPackage}/bin" "${cfg.dataDir}/nix-postgresql-bin"
