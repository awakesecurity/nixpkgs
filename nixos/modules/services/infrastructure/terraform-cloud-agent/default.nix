...
in
{
  options.services.terraform-cloud-agent = {
    ...
    tokenPath = lib.mkOption {
      ...
      description = "Path to the Terraform Cloud token this agent should use.";
      example = "/etc/terraform-cloud-agent/token";
    };

    hooks = lib.mkOption {
      description = "Set of hooks the TFC agent should run (if any).";
      default = {};
      type = lib.types.submodule {
        options = {
          pre-plan = lib.mkOption {
            ...
            default = null;
            description = "Terraform pre-plan hook script.";
          ...
          post-plan = lib.mkOption {
            type = lib.types.nullOr packageWithMain;
            default = null;
            description = "Terraform post-plan hook script.";
          };

          pre-apply = lib.mkOption {
            type = lib.types.nullOr packageWithMain;
            default = null;
            description = "Terraform pre-apply hook script.";
          };

          post-apply = lib.mkOption {
            type = lib.types.nullOr packageWithMain;
            default = null;
            description = "Terraform post-apply hook script.";
          };
        };
      };
    };
    ...
    user = lib.mkOption {
      type = lib.types.str;
      default = "tfc-agent";
      description = "User account under which terraform-cloud-agent runs.";
    };

    group = lib.mkOption {
      type = lib.types.str;
      default = "tfc-agent";
      description = "Group account under which terraform-cloud-agent runs.";
    };

    flags = lib.mkOption {
      description = "Set of NixOS options that are used as flags to the tfc-agent executable.";
      type = lib.types.submodule {
        ...
        options = {
          name = lib.mkOption {
            type = lib.types.str;
            description = "Name of the tfc-agent process.";
          ...
          log-level = lib.mkOption {
            ...
            default = null;
            description = "Level at which log messages should be emitted.";
            example = "warn";
          };
          ...
          address = lib.mkOption {
            ...
            default = null;
            description = "HTTP or HTTPS address of the Terraform Cloud API.";
          ...
          accept = lib.mkOption {
            ...
            description = "Optional string of comma-separated job types that this agent may run. Acceptable job types are 'plan', 'apply', 'policy', and 'assessment'. Do not put whitespace in between entries.";
            example = "plan,apply";
          };
          ...
          auto-update = lib.mkOption {
            ...
            readOnly = true;
            default = "disabled";
            description = "Controls automatic core updates behavior. Not acceptable in NixOS, so disabled.";
          };
        };
      };
    };
